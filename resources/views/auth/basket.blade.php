...
@section('content')
<div class="w-full max-w-[90%] lg:max-w-[80%] mx-auto px-4 sm:px-6 lg:px-8 py-8">
    <h1 class="text-2xl font-bold mb-6 text-gray-200">Basket</h1>
    @include("components.success")
    <div class="overflow-x-auto bg-slate-950 shadow-md rounded-lg">
        <div class="w-full flex justify-end items-center mb-2"><a href="{{ route("shop") }}" class="bg-blue-600 text-white font-semibold text-xl py-1 px-2 rounded-md hover:bg-blue-700 transition ease-in-out duration-200">Go to Shop</a></div>
            ...
            <tbody class="bg-slate-800 divide-y ">
                @foreach ($baskets as $basket)
                    <tr class="hover:bg-slate-900 transition ease-in-out duration-700 cursor-pointer">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-lg text-gray-200">{{ $basket->product->name }}</td>
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-700">
                            <form action="" method="post">
                                @csrf
                                <button type="submit" class="bg-blue-500 hover:bg-blue-700 transition ease-in-out duration-200 py-1 px-2 rounded-full text-white font-semibold" disabled={{ $basket->isPaid }}>Pay ${{ $basket->amount }}</button>
                            </form>
                        </td>
